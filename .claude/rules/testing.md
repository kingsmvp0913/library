...
2. **一旦建了 partial unique index，`<欄位> IS NOT NULL AND <比較>` 會查不到列** —— 拿掉冗餘的 `IS NOT NULL`（語意等價）。
   ...
   > (b) ⚠️ **列被 UPDATE 到離開 partial index 之後，查詢會漏掉那一列。** 兩次實測到的範圍：
   >   - **多表 JOIN 整個漏掉**（2026-08-13）——`SELECT *`／`COUNT(*)`／`WHERE returned_at IS NOT NULL` 都查得到，只有 JOIN 版回 0 筆，已歸還的紀錄在畫面上憑空消失。
   >   - **連單表的 `WHERE <索引欄位> = $1` 也漏掉**（同日稍後）——`SELECT COUNT(*) FROM loans WHERE copy_id = $1` 在歸還後回 0，害「有借閱紀錄的冊不可刪除」這條防護靜默失效。
   >   判定法：把該 index 拿掉再跑一次，正常了就是它。
   >
   > **本專案的兩個對策**（都不是為了遷就測試而改壞正式 SQL，兩種寫法在真 PG 等價；若堅持原寫法，這些功能在測試裡永遠驗不到）：
   >   - `GET /api/loans` 不用 JOIN，分開查再於 JS 合併（`routes/scan.js`）
   >   - 查某一冊的全部借閱紀錄用 `copy_id IN (SELECT id FROM copies WHERE id = $1)`（`routes/copies.js`）
   >
   > **凡是要 JOIN `loans`、或用 `copy_id` 等值查詢「含已歸還」的資料，都會踩到同一個坑。**
   > 反過來說，只查**未歸還**的列（例如 `openLoanOf`）不受影響——那些列還在索引裡。
